...
void vider_liste_contrainte(listeContrainte l)
{
	if(l != NULL)
	{
		vider_liste_contrainte(l->suivant); // vide recursivement la suite de la liste en premier
		videArbre(l->a);
...
{
	int i=1;
	fprintf(fileToWrite, "Debut d'affichage d'un ensemble de contraintes");
	while(l != NULL)
	{
		fprintf(fileToWrite, "\nPresence de variables dans contrainte %d: %s\n", i, l->presenceVariable);
		fprintf(fileToWrite, "Arbre de contrainte %d:\n", i++);
		afficheArbre(l->a, 0);
		l = l->suivant;
	}
	fprintf(fileToWrite, "Fin d'affichage d'un ensemble de contraintes\n");
}
